...
**Route-derived stops away (`stops_away`):** `attachRouteGeometry()` also records `myStop.upstreamStopCums` — the cumulative route positions of the stops *before* the student's own stop (determined by index in the ordered checkpoints, robust to sparse `runStopSeq`). On each `NewLocation`, `MqttBridge._publishStopsAway()` counts how many of those still lie ahead of the live bus (`cum > cumBus`), so the value is GPS-truthful and counts down live rather than being a schedule-clock estimate (which would freeze at 0 after a run ends and mis-report on a late run). Route mode only — publishes `None` when geometry is unavailable, so it is silent (unknown) outside an active, on-route run rather than confidently wrong.
